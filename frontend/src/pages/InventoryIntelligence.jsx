...
export const InventoryIntelligence = () => {
  ...
  const handlePredict = async (e) => {
    ...
    console.log("Predict Stock Requirement button clicked");
    setLoading(true);
    setError('');
    setResult(null);
    ...
    try {
      const response = await axios.post('https://smart-logistics-backend-mefz.onrender.com/api/inventory', payload);
      console.log("Response data:", response.data);

      if (response.data && response.data.success) {
        const data = response.data;
        const cleanCategory = (category).replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        const targetMonthName = months[monthNumber - 1] || "September";

        // Extract response data
        const required_stock = data.required_stock !== undefined ? data.required_stock : 0;
        const inventory_status = data.inventory_status || "Low";
        const safety_stock = data.safety_stock !== undefined ? data.safety_stock : 0;
        const restocking_priority = data.restocking_priority || "Low";
        const warehouse = data.warehouse || "Regional Warehouse";
        const confidence_score = data.confidence_score !== undefined ? data.confidence_score : 90;
        const last_updated = data.last_updated || new Date().toLocaleString();
        
        let historical_stock = data.historical_stock || [];
        let warehouse_distribution = data.warehouse_distribution || [];
        ...
        const errMsg = response.data.error || 'Failed to calculate inventory intelligence.';
        setError(errMsg);
      }
    } catch (err) {
      console.error("Axios errors:", err);
      const errMsg = err.response?.data?.error || 'Failed to connect to the inventory intelligence engine.';
      setError(errMsg);
    } finally {
      setLoading(false);
    }
  ...
  const getInventoryStatusBadgeColor = (status) => {
    switch (status) {
      case 'Low':
  ...
  return (
    <div className="flex-1 p-3 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6 bg-transparent text-black font-sans relative z-10">
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>Predict Stock Requirement</span>
                  </>
                )}
              </button>
          ...
          <AnimatePresence mode="wait">
            {loading && (
              ...
              >
                ...
                <div>
                  <h4 className="text-sm font-black text-black uppercase tracking-wide">Analyzing historical inventory movement...</h4>
                  <p className="text-xs text-slate-500 font-bold mt-1 max-w-sm">
                    Predicting optimal stock levels...
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {!loading && !result && (
              ...
              >
                <div className="w-16 h-16 rounded-full bg-slate-100 border-2 border-black flex items-center justify-center text-slate-500 mb-4 shadow-[3px_3px_0px_rgba(0,0,0,1)]">
                  <Warehouse className="text-2xl animate-pulse text-slate-500" />
                </div>
                <h4 className="text-sm font-black text-black uppercase tracking-wide mb-1">Stock Prediction Waiting</h4>
                <p className="text-xs text-slate-500 font-bold max-w-md leading-relaxed">
                  Configure your prediction criteria on the left and invoke the ML model to compile inventory planning metrics.
                </p>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-[#C7D2FE] border-2 border-black p-4 rounded-2xl flex flex-col justify-between h-28 shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-black">
                    <span className="text-[9px] font-black uppercase text-black tracking-wider flex items-center gap-1">
                      <Package className="text-[10px]" />
                      <span>Required Stock</span>
                    </span>
                    <div>
                      <h4 className="text-lg font-black text-black leading-none">{result.required_stock.toLocaleString()}</h4>
                      <span className="text-[8px] font-bold text-slate-700 mt-1 block">Recommended units</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#A7F3D0] border-2 border-black p-4 rounded-2xl flex flex-col justify-between h-28 shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-black">
                    ...
                    <div>
                      <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-full inline-block mb-1.5 ${getInventoryStatusBadgeColor(result.inventory_status)}`}>
                        {result.inventory_status}
                      </span>
                      <span className="text-[8px] font-bold text-slate-700 block">Current stock level</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#FDE68A] border-2 border-black p-4 rounded-2xl flex flex-col justify-between h-28 shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-black">
                    ...
                    <div>
                      <h4 className="text-lg font-black text-black leading-none">{result.safety_stock.toLocaleString()}</h4>
                      <span className="text-[8px] font-bold text-slate-700 mt-1 block">Buffer stock level</span>
                    </div>
                  </div>

                  {/* Restocking Priority */}
                  <div className="bg-[#FECDD3] border-2 border-black p-4 rounded-2xl flex flex-col justify-between h-28 shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-black">
                    <span className="text-[9px] font-black uppercase text-black tracking-wider flex items-center gap-1">
                      <AlertTriangle className="text-[10px]" />
                      <span>Restocking Priority</span>
                    </span>
                    <div>
                      <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-full inline-block mb-1.5 ${
                        result.restocking_priority === 'High' ? 'bg-red-200 border border-black text-black' :
                        result.restocking_priority === 'Medium' ? 'bg-orange-200 border border-black text-black' :
                        'bg-emerald-100 border border-black text-black'
                      }`}>
                        {result.restocking_priority}
                      </span>
                      <span className="text-[8px] font-bold text-slate-700 block">Replenishment urgency</span>
                    </div>
                  </div>

                  {/* Warehouse Allocation */}
                  <div className="bg-[#E9D5FF] border-2 border-black p-4 rounded-2xl flex flex-col justify-between h-28 shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-black">
                    <span className="text-[9px] font-black uppercase text-black tracking-wider flex items-center gap-1">
                      <Warehouse className="text-[10px]" />
                      <span>Warehouse Allocation</span>
                    </span>
                    <div>
                      <h4 className="text-xs font-black text-black leading-none uppercase truncate">{result.warehouse}</h4>
                      <span className="text-[8px] font-bold text-slate-700 mt-2 block">Primary facility</span>
                    </div>
                  </div>

                  {/* Confidence Score */}
                  <div className="bg-[#FBCFE8] border-2 border-black p-4 rounded-2xl flex flex-col justify-between h-28 shadow-[2px_2px_0px_rgba(0,0,0,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-black">
                    <span className="text-[9px] font-black uppercase text-black tracking-wider flex items-center gap-1">
                ...
                <div className="glass-panel p-5 rounded-2xl border-3 border-black bg-white shadow-lg space-y-3">
                  <div className="flex justify-between items-center pb-2 border-b border-black">
                    <span className="text-[10px] font-black uppercase text-black">Inventory Parameters</span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs font-bold text-black">
                     ...
                     <div>
                       ...
                       <span className="text-black uppercase">Extra Trees Regressor</span>
                     </div>
                  </div>
                </div>
      ...
      {result && !loading && (
        ...
        >
          ...
          <div className="glass-panel p-5 rounded-2xl border-3 border-black bg-white shadow-lg flex flex-col justify-between h-[300px]">
            <span className="text-[9px] font-black uppercase text-slate-500 mb-3 block">Inventory Trend</span>
            <div className="flex-1 w-full text-black text-[10px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={result.historical_stock} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                  <XAxis dataKey="month" stroke="#000" />
                  <YAxis stroke="#000" />
                  <Tooltip contentStyle={{ backgroundColor: '#fff', borderColor: '#000', borderWidth: '2px', color: '#000' }} />
                  <Line type="monotone" dataKey="stock" stroke="#2563EB" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          {/* Pie Chart */}
          <div className="glass-panel p-5 rounded-2xl border-3 border-black bg-white shadow-lg flex flex-col justify-between h-[300px]">
            <span className="text-[9px] font-black uppercase text-slate-500 mb-2 block">Warehouse Distribution</span>
            <div className="flex-1 w-full text-[9px] relative flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={result.warehouse_distribution}
                    cx="50%"
                    cy="50%"
                    innerRadius={50}
                    outerRadius={75}
                    ...
                    dataKey="value"
                  >
                    {result.warehouse_distribution.map((entry, index) => (
      ...
      {/* ==================== FOURTH SECTION - Inventory Recommendations Panel ==================== */}
      {result && !loading && (
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-500 font-bold mt-0.5">
                Automated seasonal stocking suggestions and warehouse management planning
              </p>
            </div>
          </div>

          {/* Inventory Recommendations Checklist */}
      ...
      {result && !loading && (
        ...
        >
          {/* Export PDF */}
          <button
            onClick={() => triggerToast('Successfully Exported Inventory Report!')}
          ...
          {/* Download CSV */}
          <button
            onClick={() => triggerToast('Successfully Downloaded Inventory Dataset!')}
            className="flex items-center gap-2 px-5 py-2.5 bg-white hover:bg-slate-50 border-2 border-black text-black rounded-xl text-xs font-black uppercase shadow-[2.5px_2.5px_0px_rgba(0,0,0,1)] transition-all cursor-pointer"
          >
          ...
          {/* Generate AI Report */}
          <button
            onClick={() => triggerToast('Logix-AI Inventory Report compiled successfully!')}
      ...
      {result && !loading && (
        <div className="flex gap-3 items-start bg-slate-50 border-2 border-black p-4 rounded-xl text-[10px] font-bold text-black leading-relaxed shadow-[2px_2px_0px_rgba(0,0,0,1)]">
          ...
          <div>
            <span className="font-black block uppercase mb-0.5">Inventory Planning Notice:</span>
            Forecasted stock requirements utilize seasonal demand patterns mapped against historical lead times and supplier reliability. Adjust safety buffer factors to mitigate supply chain disruptions.
          </div>
        </div>
      )}
